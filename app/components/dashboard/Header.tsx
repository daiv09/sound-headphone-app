    ...
    <header className="fixed top-0 inset-x-0 z-50 border-b border-white/5 bg-zinc-950/60 backdrop-blur-2xl supports-[backdrop-filter]:bg-zinc-950/30">
      <div className="w-full px-6 md:px-12 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-2xl font-bold tracking-tighter">SOUND<span className="text-blue-500">.</span></span>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-4 rounded-full border border-white/5 bg-white/5 px-5 py-2 backdrop-blur-md hover:bg-white/10 transition-colors">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs font-medium text-zinc-300 tracking-wide">{currentData.id === 'left' ? 'L' : 'R'} Connected</span>
            </div>
            <div className="h-3 w-px bg-white/10" />
            <div className="flex items-center gap-2">
              <Battery size={13} className={currentData.stats.battery < 20 ? 'text-red-400' : 'text-zinc-400'} />
              <span className="text-xs font-medium text-zinc-300 tabular-nums">{currentData.stats.battery}%</span>
            </div>
          </div>
